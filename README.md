...
| **层级**  | **作用**             | **映射机制** | **关键意义**                |
| ------- | ------------------ | -------- | ----------------------- |
| **外模式** | 用户视图（子模式），定制数据访问权限 | 外模式/模式映射 | **逻辑数据独立性**（模式改，外模式可不变） |
| **模式**  | 全局逻辑结构（所有数据的逻辑定义）  | 无        | 数据库设计的核心蓝图              |
| **内模式** | 物理存储结构（文件、索引的存储方式） | 模式/内模式映射 | **物理数据独立性**（存储改，模式可不变）  |

> **实际应用**：  
> 
> - 修改表结构（模式）时，通过**外模式/模式映射**调整视图，应用程序无需修改。  
> - 更换存储引擎（内模式）时，通过**模式/内模式映射**保持逻辑结构不变。
...
> **参照完整性级联操作**：  
> 
> - `ON DELETE CASCADE`：删除被参照表记录时，自动删除参照表中关联记录。  
> - `ON UPDATE SET NULL`：更新被参照表主码时，参照表外码置空。  
...
| **运算**   | **符号**        | **说明**         | **示例**                           |
| -------- | ------------- | -------------- | -------------------------------- |
| **并**    | $R \cup S $   | 合并两关系的所有元组（去重） | 查询所有学生和教师 → \( 学生 \cup 教师 \)     |
...
| **除**    | $R ÷ S $       | 满足：结果关系T，使得 $` T \times S \subseteq R `$（查询“包含所有”的元组） | 查询选修了**所有课程**的学生 → $` π_{学号,课程号}(选课) ÷ π_{课程号}(课程) `$                                     |
...
- **函数依赖**：$`X \to Y`$ 表示 $`X`$ 决定 $`Y`$
- **平凡 FD**：$`Y \subseteq X`$（如 $`\text{学号} \to \text{学号}`$）
- **非平凡 FD**：$`Y \not\subseteq X`$（如 $`\text{学号} \to \text{系名}`$）
- **完全 FD**：$`X \to Y`$ 且 $`\forall X' \subset X, X' \not\to Y`$
- **部分 FD**：$`X \to Y`$ 但 $`\exists X' \subset X, X' \to Y`$
- **传递 FD**：$`X \to Y, Y \to Z`$ 且 $`Y \not\to X`$，则 $`X \to Z`$ 是传递依赖
...
> **真实案例**：  
> Amazon统计：查询延迟每增加100ms，销售额下降1%  
> Google研究：页面加载时间从1s→3s，跳出率增加32%
...
| 特性                   | 描述                      | 数学保证                                                                                                       |
| -------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------- |
| **A**tomicity（原子性）   | 事务不可分割，要么全执行要么全不执行      | $` \forall T, \, T = \{op_1, op_2, \dots, op_n\} \rightarrow \text{All or Nothing} `$                        |
| **C**onsistency（一致性） | 事务使数据库从一个一致状态转换到另一个一致状态 | $` S_{\text{initial}} \xrightarrow{T} S_{\text{final}} \,\text{s.t.}\, \text{Integrity}(S_{\text{final}}) `$ |
| **I**solation（隔离性）   | 并发事务互不干扰                | $` T_i \parallel T_j \rightarrow \text{Effect}(T_i \cap T_j) = \text{Serial}(T_i, T_j) `$                    |
| **D**urability（持久性）  | 事务提交后结果永久保存             | $` \text{Commit}(T) \rightarrow \text{Persist}(T) \,\text{even after crash} `$                               |
...
- **Undo 算法**（针对未提交事务）：  
  ...
  ```plaintext
  for each log record [T_i, X, old_value] in reverse order:
      write X = old_value
      write log [T_i, ABORT]
  ```
...
- **Redo 算法**（针对已提交事务）：  
  ...
  ```plaintext
  for each log record [T_i, X, new_value] in forward order:
      if log has [T_i, COMMIT] then write X = new_value
  ```

#### 5. **基于检查点的恢复技术**

- **检查点（Checkpoint）作用**：  
  定期将缓冲池脏页写入磁盘，并记录活跃事务列表 $` L_{\text{active}} `$。  
- **恢复流程**：  
  1. 找到最后一个检查点记录 $` C_k `$。  
  2. 仅需处理 $` C_k `$ 之后开始的事务：  
     - Redo 所有 $` \text{Commit}(T) \geq C_k `$  
     - Undo 所有未提交事务 $` T \in L_{\text{active}} `$  
- **优化效果**：避免扫描全部日志，大幅缩短恢复时间。  
...
- **排他锁 (X锁)**  
  - 事务 $T$ 对数据 $A$ 加 X 锁：$` \text{lock-X}(A) `$
  - 禁止其他事务读写：$` \forall T' \neq T,\, \text{lock}(A) \text{ denied} `$
- **共享锁 (S锁)**  
  - 事务 $T$ 对数据 $A$ 加 S 锁：$` \text{lock-S}(A) `$
  - 允许多事务同时读：$` \text{lock-S}_j(A) \text{ compatible with } \text{lock-S}_k(A) `$
  - 禁止写操作：$` \text{lock-X}(A) \text{ denied} `$
...
- **可串行化调度**：并发调度结果等价于某个串行调度
  - 形式化：$` \text{Schedule} \, S \equiv S_{\text{serial}} `$
- **冲突操作**：不同事务对同一数据的读写/写写操作
  - $` \text{Conflict} = \{ (\text{read}_i(x), \text{write}_j(x)) \mid i \neq j \} \cup \{ (\text{write}_i(x), \text{write}_j(x)) \} `$

#### 2. **冲突可串行化**

- **判定方法**：构造前驱图（Precedence Graph）
  - 顶点：事务 $`\{T_1, T_2, ..., T_n\}`$
  - 边：$` T_i \to T_j `$ 当且仅当存在冲突操作 $`o_i \prec o_j`$
- **定理**：调度是冲突可串行化 $`\iff`$ 前驱图无环
